...
class LoginApp(ctk.CTk):
    ...
    def login_authenticate(self):
        ...
        try:
            ...
            if result:
                role = result[3]  # assuming role is in column 4, adjust if needed
                messagebox.showinfo("Login Success", f"Welcome, {role.capitalize()}!")
                # Redirect to respective dashboard
                if role == "admin":
                    self.show_admin_dashboard()  # placeholder
                else:
                    self.show_user_dashboard()  # placeholder
            else:
    ...
    def register_user(self):
        ...
        try:
            ...
            # Insert into users with default role_id (2 = student)
            insert_query = """
                INSERT INTO users (first_name, last_name, email, password, mobile, student_id, role_id)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
            """
            values = (first_name, last_name, email, password, mobile, student_id, 2)
